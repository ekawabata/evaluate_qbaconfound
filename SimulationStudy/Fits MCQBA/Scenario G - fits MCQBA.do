/***********************************************************************************************************************************
                       APPLIES A MONTE CARLO BIAS ANALYSIS TO DATA WITH A SURVIVAL OUTCOME AND CONTINUOUS UNMEASURED CONFOUNDER

APPLICABLE WHEN:
	ANALYSIS OF INTEREST IS A COX PH REGRESSION WHERE EXPOSURE X IS INCLUDED LINEARLY (E.G., NO ITERACTIONS WITH X)
...
tempvar xb_utilde sim_utilde offset
...
* EXTRACTING AND DEFINING FOR PRIOR DISTRIBUTION FOR eta OR ITS PRECISION
if `uniform'==1 {
...
forvalues t=1(1)`numMCreps' {
	...
	*****************************************************************/
	* SET UP CONSTRAINT
	replace `offset' = scalar(betastar_ucon)*`sim_utilde'
	
	* FIT ANALYSIS OF INTEREST
	capture stcox `exposurelist' `confounderlist', nolog offset(`offset')
