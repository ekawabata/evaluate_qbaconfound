...
PRIOR DISTRIBUTION FOR THE INTERCEPT OF LOGISTIC REGRESSION ubinM=1|X,C
...
local sd_alphaM_xnever 0.1081
	* INTERCEPT FROM UNMEASURED CONFOUNDER MODEL FOR ubinM
local mean_alphaM_cons -0.9175
local sd_alphaM_cons 0.04286
...
forvalues t=1(1)`numMCreps' {
	...
	* DRAW VALUE FOR INTERCEPT OF LOGISTIC REGRESSION ubinM=1|X,C
	scalar alphaMstar_cons = rnormal(`mean_alphaM_cons', `sd_alphaM_cons')
	...
	* SIMULATE sim_utildeM 
	quietly replace `xb_utildeM' = scalar(alphaMstar_cons) + `xb_utildeM'
