...
ALL PRIOR DISTRIBUTIONS ARE CENTRED AT A VALUE THAT IS NOT THE OBSERVED DATA ESTIMATE 
...
local mean_alphaE_x3to4 = 0.7*-65.37   
local mean_alphaE_xatleast5 = 0.7*-94.71   
local mean_alphaE_xformer = 0.7*69.72   
local mean_alphaE_xnever = 0.7*-43.47  
local sd_alphaE_x3to4 18.07   
local sd_alphaE_xatleast5 19.64  
local sd_alphaE_xformer 18.63
local sd_alphaE_xnever 27.21
	* etaE
local acc_luni_etaE 372.6		// LOWER END POINT OF ACCURATE PRIOR
local acc_uuni_etaE 716.5		// UPPER END POINT OF ACCURATE PRIOR
local acc_mean_etaE = 0.5*(`acc_uuni_etaE' + `acc_luni_etaE')
local acc_var_etaE = (1/12)*(`acc_uuni_etaE' - `acc_luni_etaE')^2
local mean_etaE = 0.7*`acc_mean_etaE'
local luni_etaE = `mean_etaE' - 0.5*(`acc_uuni_etaE'-`acc_luni_etaE')
local uuni_etaE = `mean_etaE' + 0.5*(`acc_uuni_etaE'-`acc_luni_etaE')
local var_etaE = (1/12)*(`uuni_etaE' - `luni_etaE')^2
	* COEFFICIENTS OF EXPOSURE IN UNMEASURED CONFOUNDER MODEL FOR ubinM
local mean_alphaM_x3to4 = 0.7*0.3831   
local mean_alphaM_xatleast5 = 0.7*0.3785   
local mean_alphaM_xformer = 0.7*-0.07963  
local mean_alphaM_xnever = 0.7*0.4524   
...
local acc_luni_piM 0.2900		// LOWER UPPER END POINT OF ACCURATE PRIOR
local acc_uuni_piM 0.3484		// UPPER END POINT OF ACCURATE PRIOR
local acc_mean_piM = 0.5*(`acc_uuni_piM' + `acc_luni_piM')
local acc_var_piM = (1/12)*(`acc_uuni_piM' - `acc_luni_piM')^2
local mean_piM = 0.7*`acc_mean_piM'
local luni_piM = `mean_piM' - 0.5*(`acc_uuni_piM'-`acc_luni_piM')
local uuni_piM = `mean_piM' + 0.5*(`acc_uuni_piM'-`acc_luni_piM')
local var_piM = (1/12)*(`uuni_piM' - `luni_piM')^2
	* COEFFICIENT OF uconE IN SUBSTANTIVE ANALYSIS MODEL
local mean_beta_uconE = 0.7*0.006931 
local sd_beta_uconE 0.0002805
	* COEFFICIENT OF ubinM IN SUBSTANTIVE ANALYSIS MODEL
local mean_beta_ubinM = 0.7*-1.312 
...
local resultsfile `"Monte Carlo\mcqba_`numMCreps'reps_inaccurate_piM"'
