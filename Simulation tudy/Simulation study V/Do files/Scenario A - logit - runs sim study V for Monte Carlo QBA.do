...
*************************************/
if `betaxbin'==0.5 local txt_betaxbin "05"
else if `betaxbin'==0 local txt_betaxbin "0"

local resultsfile ///
`"Simulation study V\Results\ScenarioA_logit_`samplesize'obs_`priortxt'inf_uniform`uniform'_`numMCreps'reps_strengthUC`strength_UC'_betaxbin`txt_betaxbin'.dta"'
...
quietly {   
	...
	* EXPORT
	save "Simulation study V\Data\dataset_`dataset'.dta", replace
