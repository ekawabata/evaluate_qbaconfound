...
USES ORIGINAL BIAS MODEL p(Y|X,C,sim_U1,sim_U2)p(sim_U1|X)p(sim_U2|X)
	
FITTED TO DATA SIMULATED FROM A DGM WHERE THE TRUE VALUES OF THE UNMEASURED CONFOUNDER MODEL ARE FROM A MULTIVARIATE NORMAL MODEL. 
...
*************************************************************************************************************************************************/
args samplesize numsimdatasets numMCreps priorlevel uniform strength_UC betaxcon
...
noisily di "True exposure effect is `betaxcon'"
...
*************************************/
if `betaxcon'==0.5 local txt_betaxcon "05"
else if `betaxcon'==0 local txt_betaxcon "0"

local resultsfile ///
	`"Simulation study V\Results\ScenarioB_regress_`samplesize'obs_`priortxt'inf_uniform`uniform'_`numMCreps'reps_strengthUC`strength_UC'_betaxcon`txt_betaxcon'.dta"'
...
forvalues dataset=1(1)`numsimdatasets' {
	noisily di ///
    "YconXconUcon; regress; strengthUC=`strength_UC'; `priortxt' informative prior; Uniform=`uniform'; `numMCreps' MCQBA reps: Processing dataset `dataset' of sample size `samplesize'"	
...
quietly {   
	...
	* FULL ANALYSIS MODEL
	capture regress ycon `exposurelist' `confounderlist' ucon1 ucon2
	scalar ycon_rc = _rc
	if ycon_rc==0 {
